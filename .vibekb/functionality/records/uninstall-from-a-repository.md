...
area: integration
summary: `vibekb uninstall` removes VibeKB from a repository, ownership-aware. Driven by the `.vibekb/install.json` manifest, it deletes VibeKB-owned files (everything under `.vibekb/` and namespaced adapters) and strips only VibeKB's managed block from shared files (AGENTS.md, CLAUDE.md), preserving everything else. `--keep-knowledge` retains the model records; `--dry-run` previews.
status: implemented
...
pre-existing. Finally it removes `.vibekb/` — or, with `--keep-knowledge`, removes
only `runtime/`, `reference/`, `prompts/`, `generated/`, `backups/`, and
`install.json`, retaining the model records. Files with malformed markers are left
untouched and reported. `--dry-run` previews every action.
...
- `internal/installer/uninstall.go` — the manifest-driven removal, flags, and report.
